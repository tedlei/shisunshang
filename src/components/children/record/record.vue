  ...
  <div style="height: 100%">
    <div class="navigation" v-show="!this.$route.query.recordid">
      <van-tabs v-model="active" @click="getNum">
        <van-tab v-for="(item,index) in navItems" :key="index" :title="item.title">
          <!-- 内容 {{ item }} -->
        </van-tab>
      </van-tabs>
    </div>
    <div class="content" :style="this.$route.query.recordid ? '':'margin-top: 0.54rem'">
      <div
        class="moneybox"
        :style="this.$route.query.recordid ? {top:43+'px'}:''"
  ...
  methods: {
    async getMoney() {
      ...
      let Money = {
        ...
        page_size: 20,
        type:
          this.$route.query.recordid == undefined
            ? this.active
    ...
    getData() {
      let ad_data = {
        ...
        page: 0,
        page_size: 10,
        type: this.navItems[this.active].type
      };
