  ...
  <div class="content">
      ...
      <router-link :to="{path:'/mine/Address',query:{id: 'makeorder'}}" class="common commonTwo">
        <div class="left">
          ...
          <span>
            ...
            <p
              style="color:red"
              v-if="orderData.address_default=='' && $store.getters.getreceivingAddress.name==undefined"
            >请选择收货地址</p>
    ...
    <div class="common_box allmsg" v-for="(shop,index) in orderData.shops" :key="index">
      ...
      <router-link to="business/storemsg" class="common m_b_10">
        <div class="left">
          ...
          <span>
            <p class="fontWrap fontWrapOne">{{shop.shop.shop_name}}</p>
          </span>
        </div>
        <div class="right">
          <i class="el-icon-arrow-right"></i>
      ...
      <router-link
        ...
        v-for="(goods,index1) of shop.goods"
        :key="index1"
      >
        <div class="card">
          <van-image width="1rem" height="1rem" fit="cover" :src="goods.imgsrc" />
          <div class="cardMsg">
            <p class="fontWrap fontWrapTwo">{{goods.name}}</p>
            <span class="fontWrap fontWrapOne">
              购买方式：
    ...
    <div style="padding-bottom: 0.5rem;" class="common_box allmsg">
      <div class="common delivery">
        <div class="left">配速方式</div>
        <div class="right">物流配送</div>
      </div>
      ...
      <router-link :to="{path:'/mine/invoice',query:{state:'0'}}" class="common m_b_10">
        <div>发票信息</div>
        <div class="right">
      ...
      <a class="common m_b_10">
        <div>充值金</div>
        <div class="right">
          <span style="color:#999999;" class="Check">
            可抵扣{{
            orderData.user_money>orderData.order_pay_price?orderData.order_pay_price:orderData.order_pay_price-orderData.user_money
            }}元
          </span>
          <!-- {{orderData.user_money='0.00'?'我是0':'bushi'}} -->

          <van-switch
            v-show="orderData.user_money>0?true:false"
            v-model="checked"
            size="0.2rem"
            active-color="#009900"
    ...
    <div class="clearfix bot_submit">
      ...
      <div class="right_btn" @click="uploadOrder">提交订单</div>
    </div>
  ...
  methods: {
    ...
    tjdd() {
      if (this.$route.query.iscart == 1) {
        ...
        let buy_type = this.$route.query.buy_type;
        let address_id = this.$store.getters.getreceivingAddress.id
          ? this.$store.getters.getreceivingAddress.id
          : this.orderData.address_default.id;
