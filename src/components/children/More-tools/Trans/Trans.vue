    ...
    <div class="exchage_box">
      <div class="common_box">
        <van-radio-group v-model="radio" @change="change">
          <van-radio
            :name="index"
            checked-color="#009900"
            v-for="(item,index) in tanslist"
            :key="index"
          >
            <div class="right_box">
              <div class="money_box">
              ...
              <van-icon name="down" style="transform: rotate(-90deg);" class="clo-9" />
              <div class="money_box">
                <span>至</span>
                <div>
      ...
      <van-form @submit="onSubmit">
        ...
        <div style="margin: 0 0.1rem">
          <van-button block type="info" native-type="submit" class="common_btn">立即转账</van-button>
        </div>
      </van-form>
    </div>
    <div style="padding:0.1rem 0.1rem;">
      <van-button block @click="tapMoney" class="common_btn">充值金转账</van-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getmsg: function() {
      ...
      this.$post("/api/v1/UserTrans", msg)
        .then(response => {
          ...
          this.bili =
            this.tanslist[0].form_name +
            "转" +
            this.tanslist[0].goto_name +
            "为1：" +
            this.tanslist[0].limit_multiple;
        })
    ...
    change: function(e) {
      this.bili =
        this.tanslist[e].form_name +
        "转" +
        this.tanslist[e].goto_name +
        "为1：" +
        this.tanslist[e].limit_multiple;
    ...
    //跳转到充值金转账
    tapMoney(){
      this.$router.push({path:'/mine/transferMoney'})
    },
  },
  mounted() {
    this.getmsg();
