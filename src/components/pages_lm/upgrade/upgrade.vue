  ...
  <div class="content upg_app">
    ...
    <div class="upg_list">
        <template v-for="(item,i) of topUpList">
            <upgList class="list_set" :isShowRicon=item.isShowRicon :item=item :topUpNum="topUpNum" 
                :i='i' :key="i" :userMoney="userMoney" @tapSel="tapSel"></upgList>
        </template>
    </div>
    
    <div class="upg_check">
        <van-checkbox checked-color="#07c160" v-model="autoHmoney">
            <span style="font-size:0.15rem;color:#999;">会员到期自动续费一个月 可随时取消</span>
        </van-checkbox>
    </div>

    <div class="upg_btn_kt">
        <div class="btn" @click="tapKthu">
            开通会员
        </div>
    </div>
    ...
    <div class="upg_explain" 
        ...
        <div class="upg_p_list">
            ...
            <p class="upg-l-p">会员：升级费90元(平台赠送200签到现金)</p>
        </div>
        <div class="upg_p_list">
            <div class="upg_dd"></div>
            <p class="upg-l-p">代理：升级费420元(平台赠送1000签到现金)</p>
        </div>
        <div class="upg_p_list">
            <div class="upg_dd"></div>
            <p class="upg-l-p">合伙：升级费1080元(平台赠送3000签到现金)</p>
        </div>
    </div>
...
import upgList from './upgList.vue';
  ...
  methods: {
    ...
    tapKthu(){
        ...
        let _this = this;
        if(!isTapbtn) _this.$notify({ type: 'success', message: '请勿重复点击' });
        if(topUpList[topUpNum].is_prohibit){
            _this.$notify({ type: 'success', message: '您已开通此等级或之上的会员，请勿重复选择' });
            return
        }
